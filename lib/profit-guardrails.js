export const DEFAULT_PROFIT_POLICY = Object.freeze({
  ...
  currency: "USD",
  // When true the minimum-margin floor is a hard launch block. When false the
  // floor is advisory: margin/profit are still calculated and shown, and a
  // below-floor "blocked" status does not stop launch or force price-change
  // approvals — but missing cost/profitability data still fails closed.
  ...
  return {
    ...
    enforceMinimumMargin: boolean(
      policyValue(
        ...
        DEFAULT_PROFIT_POLICY.enforceMinimumMargin
      ),
      DEFAULT_PROFIT_POLICY.enforceMinimumMargin
    )
  };
...
// Pure launch gate used by server helpers and publisher APIs.
// Advisory mode (enforceMinimumMargin === false) relaxes only the margin floor
// so a computed "blocked" status can still launch. Missing cost/profitability
// (needs_cost, null, undefined) always fails closed in both modes.
export function evaluateLaunchGate(status, policy = DEFAULT_PROFIT_POLICY) {
  const normalizedPolicy = normalizeProfitPolicy(policy);
  const missingCost = !status || status === "needs_cost";

  if (missingCost) {
    return { readyToLaunch: false, hardBlocked: true };
  }
  ...
  if (status === "blocked") {
    if (!normalizedPolicy.enforceMinimumMargin) {
      return { readyToLaunch: true, hardBlocked: false };
    }
    return { readyToLaunch: false, hardBlocked: true };
  }
