...
export async function getCatalogVariantPrices(variantId) {
  return printfulRequest(
    `/v2/catalog-variants/${encodeURIComponent(variantId)}/prices`
  );
}
...
export function artworkIsReady(variant, requiredTypes = ["front"]) {
  const files = Array.isArray(variant?.files)
    ? variant.files.filter(
        (file) => file?.id || file?.url || file?.preview_url
      )
    : [];
  ...
  return requiredTypes.every((requiredType) =>
    files.some((file) => {
      const fileType = String(
        file?.type || file?.placement || file?.type_name || ""
      ).toLowerCase();
      const required = String(requiredType || "").toLowerCase();

      if (required === "embroidery_front") {
        return (
          fileType === "default" ||
          fileType === "front" ||
          fileType.startsWith("embroidery_front")
        );
      }

      if (required === "front") {
        return fileType === "front" || fileType === "default";
      }

      return fileType === required;
    })
  );
}
...
export function chooseCatalogVariant(
  ...
  const colorOnly = available.find(
    (variant) =>
      normalizeText(variant?.color) ===
      normalizeText(requestedColor)
  );
