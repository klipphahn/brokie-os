...
// Decides whether a product can launch given its profitability status and the
// policy. In advisory mode (enforceMinimumMargin === false) the margin floor
// never blocks a launch, so prices can be set freely like the Printful site.
function evaluateLaunchGate(status, policy) {
  const enforced = policy?.enforceMinimumMargin !== false;
  if (!enforced) {
    return { readyToLaunch: true, hardBlocked: false };
  }
  return {
    readyToLaunch: ["ready", "warning"].includes(status),
    hardBlocked: !status || ["blocked", "needs_cost"].includes(status)
  };
}
...
export async function estimatePrintfulProductionCost(
  ...
  if (!catalogProductId && selectedVariantIds.length) {
    try {
      const variantPricing = await getCatalogVariantPrices(
        selectedVariantIds[0]
      );
      catalogProductId = Number(
        variantPricing?.data?.product?.id || 0
      );
      if (catalogProductId) {
        costDiscoverySource = "printful_variant_pricing_api";
      }
    } catch (error) {
      syncLookupError = syncLookupError || error.message;
    }
  }
...
async function ensurePriceApproval(
  ...
  // Advisory mode never forces a price change. Clear any stale pending request
  // so the approval queue does not keep nagging about a manually chosen price.
  if (profitability.policy?.enforceMinimumMargin === false) {
    ...
    if (existing) {
      const { error: updateError } = await supabase
        ...
        .update({
          status: "superseded",
          summary: "Guardrail is in advisory mode; price is set manually.",
...
export async function refreshProductProfitability(
  ...
  const gate = evaluateLaunchGate(saved.status, policy);

  return {
    policy,
    profitability: saved,
    approval,
    readyToLaunch: gate.readyToLaunch,
...
export async function loadProductGuardrail(supabase, productId) {
  ...
  const gate = evaluateLaunchGate(profitability?.status, policy);
  return {
    policy,
    profitability,
...
export async function loadGuardrailOverview(supabase) {
  ...
  return {
    policy,
    advisory: policy.enforceMinimumMargin === false,
    items,
