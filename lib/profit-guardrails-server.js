...
async function ensurePriceApproval(
  ...
  // Advisory mode does not force a price change for a below-floor margin.
  // Clear any stale pending request so the approval queue does not keep
  // nagging about a manually chosen price. Missing cost still blocks launch.
  if (profitability.policy?.enforceMinimumMargin === false) {
    ...
    if (existing) {
      const { error: updateError } = await supabase
        ...
        .update({
          status: "superseded",
          summary: "Margin floor is advisory; price is set manually.",
