...
export const DEFAULT_DISCORD_HEARTBEAT_STALE_MS = 15 * 60 * 1000;
...
async function collectDiscordBot({
  fetchImpl,
  discordBotUrl,
  discordBotHeartbeat,
  discordHeartbeatStaleMs,
  timeoutMs,
  now
}) {
  if (discordBotHeartbeat?.configured) {
    const checkedAt = now().getTime();
    const updatedAt = Date.parse(discordBotHeartbeat.updatedAt || "");
    if (discordBotHeartbeat.error) {
      return settledCheck({
        ...
        httpStatus: null,
        status: "degraded",
        detail: "Discord bot operations heartbeat is unavailable."
      });
    }
    if (!Number.isFinite(updatedAt)) {
      return settledCheck({
        ...
        httpStatus: null,
        status: "degraded",
        detail: "Discord bot operations heartbeat has not been received."
      });
    }
    ...
    const staleMs = Math.max(
      1,
      Number(discordHeartbeatStaleMs) || DEFAULT_DISCORD_HEARTBEAT_STALE_MS
    );
    return settledCheck({
      id: CHECK_IDS.discordBot,
      label: "Discord bot",
      ...
      finishedAt: checkedAt,
      httpStatus: null,
      status: ageMs <= staleMs ? "healthy" : "degraded",
      detail:
        ageMs <= staleMs
          ? "Discord bot operations heartbeat is current."
          : "Discord bot operations heartbeat is stale."
    });
  }
...
export async function collectEcosystemHealth({
  ...
  discordBotUrl = null,
  discordBotHeartbeat = null,
  discordHeartbeatStaleMs = DEFAULT_DISCORD_HEARTBEAT_STALE_MS,
