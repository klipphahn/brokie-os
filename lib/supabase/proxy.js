...
export async function updateSession(request) {
  ...
  // These routes handle their own authorization or do not require a browser
  // session. Mobile routes validate Supabase bearer tokens in the route handler
  ...
  if (isPublicPath(pathname)) {
    if (pathname === "/login" && user && isAuthorizedAdminEmail(user.email)) {
      const url = request.nextUrl.clone();
      url.pathname = "/";
      url.search = "";
