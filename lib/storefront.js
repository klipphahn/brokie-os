...
export const DEFAULT_STOREFRONT_SETTINGS = {
  ...
  shipping_policy_body:
    "Every Brokie item is made to order through Printful. We send tracking as soon as production finishes and the package leaves the facility.",
  returns_policy_title: "Returns",
  returns_policy_body:
    "Because each item is made to order, returns are limited to damaged, misprinted, or incorrect items. If something arrives wrong, reach out quickly and we’ll make it right.",
  fulfillment_note:
    "Printed on demand. Fulfilled by Printful. Together we win.",
...
export function storefrontPublicSettings(row = {}) {
  ...
  const managedCollectionPath = `/collections/${merged.collection_handle}`;
  const primaryCtaPath = merged.primary_cta_url === managedCollectionPath
    ? "/collections/all"
    : merged.primary_cta_url;
  return {
    ...
    hero: {
      ...
      primaryCta: {
        ...
        url: absoluteShopUrl(merged.shop_domain, primaryCtaPath)
      },
      secondaryCta: {
    ...
    collection: {
      ...
      managedUrl: absoluteShopUrl(merged.shop_domain, managedCollectionPath)
    },
