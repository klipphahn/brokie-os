...
export async function fetchBrokieAi(path, options = {}) {
  ...
  const { timeoutMs = 55000, ...requestOptions } = options;
...
export async function fetchBrokieAiAdmin(path, options = {}) {
  const adminToken = process.env.BROKIE_AI_ADMIN_TOKEN;
  if (!adminToken) {
    throw new Error("Brokie AI admin credential is not configured.");
  }

  return fetchBrokieAi(path, {
    ...options,
    headers: {
      ...options.headers,
