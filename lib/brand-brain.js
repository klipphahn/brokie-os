import { brandDna } from "./brand-dna.js";
import { merchListingCopyForProduct } from "./product-types.js";
...
function familyForProduct(product = {}) {
  return product.family || merchListingCopyForProduct(product).family;
}

function productTypeForFamily(family, products = []) {
  const match = (Array.isArray(products) ? products : []).find(
    (product) => familyForProduct(product) === family
  );
...
export function buildBrandBrain({
  ...
  for (const item of Array.isArray(products) ? products : []) {
    const family = familyForProduct(item);
  ...
  const nextFamily = queue.nextBlocked
    ? familyForProduct(queue.nextBlocked.product)
    : primaryFamily;
  ...
  const focusProductType = queue.nextBlocked?.product?.productType
    ...
    || "tee";
  const focusCopy = merchListingCopyForProduct({
    ...(queue.nextBlocked?.product || {}),
    productType: focusProductType
  });
