...
export function mockupCompositePosition(placement, artworkMetadata = {}) {
  const artworkWidth = Number(artworkMetadata.width || placement.width);
  const artworkHeight = Number(artworkMetadata.height || placement.height);

  return {
    left: Math.round(
      placement.left + (placement.width - artworkWidth) / 2
    ),
    top: Math.round(
      placement.top + (placement.height - artworkHeight) / 2
    )
  };
}
...
export function merchListingCopyForProduct(product = {}) {
  const identity = [
    product.productType,
  ...
  return merchListingCopy(identity);
}

export function resolveMerchFieldOverride({
  value,
  field,
  storedProductType,
  liveProduct
  ...
  const generatedValues = new Set(
    [
      merchListingCopy(storedProductType || "")[field],
      merchListingCopyForProduct(liveProduct)[field]
    ]
      .filter(Boolean)
      .map((item) => String(item).trim())
  );

  return generatedValues.has(candidate) ? null : candidate;
}
