...
export function merchListingCopy(productType) {
  ...
  if (key === "hoodie") {
    return {
      ...
      subtitle: "Heavyweight fleece with front chest and back statement placement.",
      badge: "PREMIUM",
  ...
  if (key === "sticker") {
    return {
      title: "Sticker",
      subtitle: "Die-cut sticker for laptops, toolboxes, and hard hats.",
      badge: "STICKER",
...
export function buildListingDefaults({
  ...
  const family = productTypeFamily(productType);
  const familyTagMap = {
    apparel: ["apparel", "streetwear", "workwear", "the brokie"],
    headwear: ["hat", "cap", "headwear", "the brokie"],
    sticker: ["sticker", "decal", "gift", "the brokie"]
  };
  const familyTitleMap = {
    apparel: "apparel",
    headwear: "hat",
    sticker: "sticker"
  };

  const productLabel = familyTitleMap[family] || "apparel";

  return {
    title,
    description: `${title} is a ${productLabel} built for the people still building. ${copy.subtitle} ${theme}`.trim(),
    seoTitle: `${title} | The Brokie`,
    metaDescription: `${title} from ${collectionName}. ${copy.subtitle}`,
    tags: familyTagMap[family] || familyTagMap.apparel,
    badge: copy.badge,
