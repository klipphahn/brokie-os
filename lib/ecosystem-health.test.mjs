...
describe("ecosystem health collection", () => {
  ...
  it("uses the private Discord operations heartbeat without exposing a bot health URL", async () => {
    const checkedAt = new Date("2026-08-19T22:30:00.000Z");
    const report = await collectEcosystemHealth({
      origin: ORIGIN,
      websiteUrl: WEBSITE_URL,
      now: () => checkedAt,
      discordBotHeartbeat: {
        configured: true,
        updatedAt: "2026-08-19T22:25:00.000Z"
      },
      env: {},
      async fetch(url) {
        const key = routeFor(url);
    ...
    const discord = report.checks.find((check) => check.id === "discordBot");
    assert.equal(discord.status, "healthy");
    assert.equal(discord.detail, "Discord bot operations heartbeat is current.");
    assert.equal(discord.httpStatus, null);
    assertNoTargetUrls(report);
  });

  it("reports stale or unavailable Discord operations heartbeats as degraded", async () => {
    const now = () => new Date("2026-08-19T22:30:00.000Z");
    ...
    const stale = await collectEcosystemHealth({
      origin: ORIGIN,
      websiteUrl: WEBSITE_URL,
      now,
      fetch,
    ...
    const unavailable = await collectEcosystemHealth({
      origin: ORIGIN,
      websiteUrl: WEBSITE_URL,
      now,
      fetch,
      env: {},
      discordBotHeartbeat: { configured: true, error: true }
    });

    assert.equal(stale.checks.find((check) => check.id === "discordBot").status, "degraded");
    assert.equal(
      unavailable.checks.find((check) => check.id === "discordBot").detail,
      "Discord bot operations heartbeat is unavailable."
    );
  });
